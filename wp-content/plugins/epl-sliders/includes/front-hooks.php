...
class EPL_SLIDER_CONFIG {
	...
	/** this function returns an array mapping actual slider configuration to that of the slider settings field names
	 *
	 *  in admin panel or custom settings
	 *
	 * in case of shortcodes
	 *
	 *  all the dynamic fields having prefix 'epl_'  should be replaced by actual values prior to making it json object & using in slider
	 *
	 */
	public function slider_config_options () {
		...
		return array(
			...
			'$AutoPlayInterval'					=> 'epl_slider_animationSpeed',                            
    ...
    /**
     * returns true if slider is running on current page
     *
     * @since 1.0.0
     */
	...
	/**
	 * checks if slider is running via shortcode
	 *
	 */
	...
	function epl_slider_enqueue_scripts() {
		...
		if ( $this->is_slider_active() ) {
			wp_enqueue_script( 'epl-slider-jssor', EPL_SLIDER_PLUGIN_URL . 'js/jssor.slider.mini.js', __FILE__  );
			wp_enqueue_script( 'epl-slider-jquery.fancybox', EPL_SLIDER_PLUGIN_URL . 'js/jquery.fancybox.pack.js', __FILE__  );
			wp_enqueue_script( 'epl-slider-script', EPL_SLIDER_PLUGIN_URL . 'js/script.js', __FILE__  );
			wp_enqueue_style( 'epl-slider-fancybox-style', EPL_SLIDER_PLUGIN_URL . 'css/jquery.fancybox.css', __FILE__  );
			wp_enqueue_style( 'epl-slider-style', EPL_SLIDER_PLUGIN_URL . 'css/style.css', __FILE__  );
		}
	}
    ...
    public function resize_image( $url, $width = null, $height = null, $crop = true, $align = 'c', $quality = 100, $retina = false, $data = array() ) {
        ...
        if ( ! file_exists( $dest_file_name ) ) {
            ...
            if ( is_wp_error( $editor ) ) {
                return new WP_Error( 'epl-slider-error-no-editor', __( 'No image editor could be selected. Please verify with your webhost that you have either the GD or Imagick image library compiled with your PHP install on your server.', 'epl' ) );
            }
    ...
    public function get_image_info( $args ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error( 'epl-slider-error-no-url', __( 'No image URL specified for cropping.', 'epl' ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'epl-slider-error-no-file', __( 'No file could be found for the image URL specified.', 'epl' ) );
        }
        ...
        if ( ! $size ) {
            return new WP_Error( 'epl-slider-error-no-size', __( 'The dimensions of the original image could not be retrieved for cropping.', 'epl' ) );
        }
	...
	function epl_slider_render_options() {
        ...
        $hey = $this->slide_get_option('epl_slider_slideshow');

		$map_js_options = array(
	
			'$AutoPlay'							=> $this->is_allowed_archives() ? 0: $this->slide_get_option('epl_slider_slideshow'),                                    
			'$AutoPlayInterval'					=> $this->slide_get_option('epl_slider_animationSpeed'),                            
			...
			'$ThumbnailNavigatorOptions'			=> array(                       
				'$Class'								=> '$JssorThumbnailNavigator$',              
				'$ChanceToShow'							=> $this->is_allowed_archives() ? 0 : $this->slide_get_option('epl_slider_use_thumbnails'),                               
				'$ActionMode'							=> 1,                                 
		...
		if ( $this->is_slider_active() ) {
			...
			if(!$this->is_slider_shortcode()) {
				...
				if($this->slide_get_option('epl_slider_use_thumbnails') == 0) { ?>
					<style>
						.epl-slider-slides {
							right:0!important;
						}
	...
	function output_css() {
		...
		if( class_exists('EPL_SLIDER_CSS') ) {
			...
			if($this->slide_get_option('epl_slider_thumb_orientation') == 1 || $this->is_allowed_archives() ) {
	...
	function epl_slider_advanced_map_box_tabs($tabs) {
		...
		$extra = array('gallery'	=>	__('Gallery','epl') );
		
		if(get_property_meta('property_video_url') != '') {
			$extra['video']	=	__('Video','epl');
		}
		$tabs = $extra + $tabs;
		return $tabs ;
	}
